...
def build_tick_prompt(
    ...
    # Name a section only when it was actually rendered into THIS prompt.
    ...
    #
    # [SESSION CANVAS] is named for the same reason [SESSION CONTEXT] is: it is
    # text the AGENT wrote on an earlier tick and the engine replays verbatim
    # into every later one. A venue claim it got wrong once therefore arrives
    # back as if it were established fact — the shape of the incident this
    # precedence line exists for (a wrong venue/pair claim repeated for six
    # ticks). Leaving a section unranked is what let the agent invent a
    # resolution last time, so the canvas is named explicitly -- and only on
    # the ticks that actually carry one (experiments keep no canvas).
    outranked = " OR ".join(
        ["the strategy instructions"]
        + (["[SESSION CONTEXT]"] if trading_context else [])
        + ([] if is_experiment else ["[SESSION CANVAS]"])
    )
    config_lines = [
        "[CURRENT CONFIG]",
        (
