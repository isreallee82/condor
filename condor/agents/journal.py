...
class JournalManager:
    ...
    def write_learning(
        ...
        * ``supersede=True`` (or a non-empty ``replaces``) -- REPLACE the entry
        ...
        # No write here on purpose. Upstream wrote (atomically, CORR-193) at
        # this point, but new_text is not final yet: anything the MAX_LEARNINGS
        # cap just pushed out still has to be parked under "## Retired
        # Insights" below. Writing twice would publish, in between, a document
        # in which the evicted safety lessons exist nowhere -- and a crash
        # between the two writes would make that permanent. The single atomic
        # write happens once new_text is complete, after the eviction block.
