...
# ``_executor_notional``).
#
# Only keys that are quote-denominated *by definition* belong here. ``amount``
# used to be in this tuple, back when the gate had no way to price anything:
# for an ``order_executor``/``position_executor`` it is a BASE quantity, so
# reading it as quote states a dollar figure nobody computed -- 653 PUMP is not
# $653. ``_planned_amount_quote`` now values those legs against a reference
# price, which is a real reading, so this one stays out. Asserting only what is
# actually known is the whole point of these guards.
_NOTIONAL_KEYS = ("total_amount_quote", "amounts_quote")


def _executor_notional(config: Any) -> tuple[float, str]:
    """Quote notional an executor create *declares*, read from its own config.
    ...
    This is the gate's own second opinion, deliberately independent of the
    priced ``planned_amount_quote`` its caller supplies: it needs no price feed,
    no executor-type table and no oracle, so it still measures a ladder if any
    of those change shape. It cannot value a base-denominated leg, so it is a
    FLOOR under the priced reading, never a replacement for it.
    ...
    if not isinstance(config, dict):
        # A create whose config we cannot even index is a create we cannot
        # size. Say so and let the caller block; do not read it as $0.
        return 0.0, f"executor_config is {type(config).__name__}, not an object"
    notional = 0.0
    for key in _NOTIONAL_KEYS:
        ...
        if not math.isfinite(total):
            return 0.0, f"{key}={raw!r} is not a finite amount"
...
class RiskEngine:
    ...
    def check_executor_action(
        ...
        ``planned_amount_quote`` is the create valued in quote by the caller
        (see :func:`_planned_amount_quote`). It is required: a create that could
        not be priced is refused, not approved against an assumed size. The gate
        additionally re-reads the config's own quote-denominated size fields and
        holds the call against whichever reading is larger, so the limit does
        not go quiet if the priced reading ever under-reports.
        ...
        # Check position size. Two independent readings; the LARGER binds.
        #
        #  1. ``planned_amount_quote`` -- the PRICED reading. The callback runs
        #     ``_planned_amount_quote``, which is the only path that can value a
        #     base-denominated leg (it fetches a reference price) and which
        #     already sums a dca_executor ladder. It is authoritative on what a
        #     create is worth in quote.
        #  2. ``_executor_notional`` -- the reading the gate takes for ITSELF,
        #     from the config's own quote-denominated size keys, scalar *and*
        #     list-valued. A dca_executor's size lives in ``amounts_quote``, a
        #     LIST; this gate used to read only scalar keys, measured every such
        #     create as $0, and max_position_size_quote never bound at all. The
        #     limit must not depend on its caller having sized the call
        #     correctly, so the sum happens here too.
        #
        # Neither reading is trusted to be the bigger one. Under-measuring is
        # precisely how this gate went inert, so a create is held against
        # whichever reading is larger -- e.g. a ladder whose ``amounts_quote``
        # sums past the ``total_amount_quote`` it declares.
        config = input_data.get("executor_config") or {}
        declared, problem = _executor_notional(config)
        if problem:
        ...
        if (
            ...
            # An unpriced create is refused, never approved blind (SEC-093).
            # ``declared`` does not rescue it: it cannot see a base-denominated
            # leg, so a config that declares nothing is not thereby worth $0.
            return False, "Planned quote exposure is unavailable"

        amount = max(float(planned_amount_quote), declared)
